...
class TemplateService:
    ...
    async def create_template(
        self, template_data: TemplateCreate, owner: User
    ) -> Template:
    ...
    async def get_template_by_id(self, template_id: uuid.UUID, owner: User) -> Template:
        result = await self.session.execute(
            select(Template).where(
                and_(Template.id == template_id, Template.owner_id == owner.id)
            )
        )
    ...
    async def update_template(
        self, template_id: uuid.UUID, template_data: TemplateUpdate, owner: User
    ) -> Template:
    ...
    async def count_user_templates(self, owner_id: uuid.UUID) -> int:
        ...
        result = await self.session.execute(
            select(func.count(Template.id)).where(Template.owner_id == owner_id)
        )
        return result.scalar() or 0

    async def get_user_templates(
        self, owner_id: uuid.UUID, limit: int | None = None
    ) -> list[Template]:
